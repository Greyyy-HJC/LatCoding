...
def create_bw_seq_Pyquda(prop, trafo, origin):
    ...
    src_seq = up_quark_insertion(prop, prop, Cg5, PpSzp)
...
smearing_input = create_bw_seq_Pyquda(prop, U_trafo, origin=[0,0,0,0])

print(np.shape(smearing_input[:]))
print(np.linalg.norm(smearing_input[:])**2)
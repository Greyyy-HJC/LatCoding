...
def save_correlator_hdf5(results_dict, filepath):
    ...
    g.message(f"Saved correlator data to {output_path}")
...
conf_path = ROOT_DIR / "configs" / f"{lat_tag}_cg"
precision = 1e-14
...
for conf_n in range(N_conf):
    ...
    conf_file = conf_path / f"wilson_b6.cg.{precision}.{conf_n}"
    if not conf_file.exists():
        raise FileNotFoundError(f"Missing gauge configuration: {conf_file}")
    g.message(f"Loading config from {conf_file}")
    U_cg = g.convert(g.load(str(conf_file)), g.double)
    # U = g.qcd.gauge.random(grid, rng)
    # U_cg, _ = g.gauge_fix(U, maxiter=5000)
    ...
    for hadron_momentum in hadron_momenta:
        ...
        srcDp = PDF_Measurement.create_src_2pt(pos, U_cg[0].grid)
        ...
        corr_2pt = PDF_Measurement.contract_2pt_PDF(prop_exact_f, phases_2pt) # shape: (polarizations, 1, gamma_list, tsep), because calculate 1 momentum at a time
        ...
        for tsep in PDF_Measurement.tsep_list:
            ...
            sequential_bw_prop_down = PDF_Measurement.create_bw_seq(
                prop_exact, prop_exact_f, 2, tsep, hadron_momentum, pos
            )
            sequential_bw_prop_up = PDF_Measurement.create_bw_seq(
                prop_exact, prop_exact_f, 1, tsep, hadron_momentum, pos
            )
...
output_file = ROOT_DIR / "gpt" / "addons" / "proton_qPDF" / "output" / f"{lat_tag}_Cg5_proton_PDF.h5"
save_correlator_hdf5(results, output_file)

g.message("PDF calculation completed!")
g.message(f"Output file saved as {output_file}")

...
class proton_qPDF:
    ...
    def contract_2pt_PDF(self, prop_f, phases):
        # g.message("Begin sink smearing")
        # prop_f = g.create.smear.boosted_smearing(prop_f, w=self.width, boost=self.pos_boost)
    ...
    def create_bw_seq(self, inverter, prop, flavor, tsep, hadron_momentum, origin=None):
        # prop = g.create.smear.boosted_smearing(prop, w=self.width, boost=self.boost_out)
        ...
        for i, pol in enumerate(self.pol_list):
            ...
            # tmp_prop = g.create.smear.boosted_smearing(smearing_input, w=self.width, boost=self.boost_out)
    ...
    def create_src_2pt(self, pos, grid):
        """Create 2pt source for propagator calculation"""
        src = g.mspincolor(grid)
        g.create.point(src, pos)
        # src = g.create.smear.boosted_smearing(src, w=self.width, boost=self.boost_in)
        return src 